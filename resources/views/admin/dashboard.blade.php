        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="grid gap-6 lg:grid-cols-2">
                <div class="rounded-3xl bg-white p-6 shadow-sm border border-slate-200">
                    <div class="text-sm font-semibold uppercase tracking-[0.24em] text-orange-600">Statistik</div>
                    <div class="mt-4 text-4xl font-bold text-slate-900">{{$artikelTotal}}</div>
                    <div class="mt-2 text-sm text-slate-500">Total artikel yang terdaftar</div>
                </div>

                <div class="rounded-3xl bg-white p-6 shadow-sm border border-slate-200">
                    <div class="text-sm font-semibold uppercase tracking-[0.24em] text-slate-600">Total user terdaftar</div>
                ...
                <div class="rounded-3xl bg-white p-6 shadow-sm border border-slate-200">
                    ...
                    <div class="mt-6 flex flex-wrap gap-3">
                        <a href="{{ route('artikel.myartikel') }}" 
                           class="inline-flex items-center rounded-md bg-orange-600 px-4 py-2 text-sm font-semibold text-white hover:bg-orange-700">
                           Artikel Saya
                        </a>
                    </div>
                ...
                @foreach ($artikel as $artikel)
                    <div class="flex flex-col justify-between rounded-xl bg-white p-6 shadow border border-slate-200">
                        ...
                        <div class="mt-4 flex gap-2">
                            ...
                            <form action="{{ route('artikel.destroy', ['artikel' => $artikel->id, 'from' => 'myartikel']) }}" method="POST" class="px-3 py-1 text-xs rounded-md bg-red-600 text-white hover:bg-pink-700">
                                @csrf
                                @method('DELETE')
